...
public class FlickTouchListener implements View.OnTouchListener {
    ...
    /** 最大移動時間 */
    protected static final long  MAX_MOVE_TIME = 150;

    private OnFlickListener      mListener;

    public interface OnFlickListener extends EventListener {
    ...
    public FlickTouchListener(OnFlickListener listener) {
        mListener = listener;
    }
    ...
    public boolean onTouch(View v, MotionEvent event){
        // タッチされている指の本数
        LogUtils.v("touch_count = " + event.getPointerCount());
    ...
    /**
     * フリック方向を検出してリスナーに通知する
     * 
     * @return フリック方向を検出できなかった場合はfalse
     */
    private boolean detectFlickDirection(){
        // 左上を原点とし、右下が正
        ...
        if(absMoveX > absMoveY){ // 左右
            if(moveX > 0){ // 左
                mListener.onFlickLeft();
            }else{ // 右
                mListener.onFlickRight();
            }
        }else{ // 上下
            if(moveY > 0){ // 上
                mListener.onFlickUp();
            }else{ // 下
                mListener.onFlickDown();
            }
        }
        return true;
    }
